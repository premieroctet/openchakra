...
class Wizard extends React.Component {
      ...
      values.submission.forEach(e => {
        ...
        axios
          ...
          .then(res => {
            ...
            axios.get(`${url}myAlfred/api/serviceUser/currentAlfred`).then(response => {
              ...
              axios
                .post(url + 'myAlfred/api/shop/add', { booking_request, no_booking_request, my_alfred_conditions, profile_picture, identity_card, recommandations, welcome_message, flexible_cancel, moderate_cancel, strict_cancel, is_particular, is_professional, self_employed, individual_company, name, creation_date, naf_ape, siret, arrayService, option_presta_user, option_presta_client, option_presta_visio })
                .then(result => {
                  ...
                  axios
                    .post(url + 'myAlfred/api/users/profile/idCard', formDataIdProfile)
                    .then(res => {})
                    .catch(err => {
                      console.log(err);
                    });
                  ...
                  if (profilePicture) {
                    ...
                    axios
                      .post(url + 'myAlfred/api/users/profile/picture', formDataPicture)
                      .then(res => {})
                      .catch(err => {
                        console.log(err);
                      });
            ...
            prestations: Yup.array().of(
              Yup.object().shape({
                ...
                billing: Yup.string().when('checked', {
                  is: true,
                  then: Yup.string()
                    .typeError('Veuillez sélectionner une méthode de facturation')
                    .required('Veuillez sélectionner une méthode de facturation'),
  ...
  render() {
    ...
    const titles = ['', 'Etape 1 - Choisissez votre catégorie puis votre service', `Etape 1 - Configuration de votre service - ${textLabel}`, 'Etape 2 - Indiquez vos disponibilités et conditions', 'Etape 2 - Indiquez vos disponibilités et conditions', 'Etape 3 - Présentez vous !'];
          ...
          <React.Fragment>
            {page !== 0 && (
              <div style={{ backgroundColor: 'white' }}>
                <h3 style={{ fontFamily: 'Helvetica', marginLeft: 10, color: 'black', paddingTop: '1.5rem' }}>{titles[page]}</h3>
                <div>
            ...
            <form onSubmit={handleSubmit}>
              <Grid container style={{ width: '100%' }}>
                <Grid className="responsiveForm" style={{ width: page === 3 ? '100%' : '50%' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexDirection: page === 0 ? 'row-reverse' : 'row', margin: '5%' }}>
                      ...
                      {page === 1 && (
                        <Button type="submit" variant="contained" color="secondary" style={{ color: 'white' }} disabled={values.submission.length <= 0}>
                          Suivant
                        </Button>
                      )}
                      {page === 2 && (
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    if (typeof form.errors.submission === 'undefined') {
                                      ...
                                      toast.error(
                                        <div>
                                          Les services suivants n'ont pas été correctement configurés :<br />
                                          {form.errors.submission.map((service, i) => {
                ...
                <Grid style={{ display: page === 3 ? 'none' : '' }} className="imgDiv">
                  <div style={{ backgroundImage: page === 0 || page === 1 || page === 2 ? 'url("../../static/Creation_shop_step1.png")' : page === 4 || page === 5 ? 'url("../../static/Creation_shop_step3.png")' : null, backgroundSize: 'contain', backgroundRepeat: 'no-repeat', backgroundPosition: page !== 3 ? 'center' : null, width: '100%', height: '100%' }}></div>
                </Grid>
              </Grid>
            </form>
          </React.Fragment>
          ...
          <Wizard.Page>
            <Grid container className={classes.cardContainer} style={{ justifyContent: 'start' }}>
              <div style={{ padding: '0rem 2rem 1rem 2rem', width: '100%' }}>
                <Typography variant="h6" style={{ marginBottom: '.5rem', marginTop: '1rem', fontSize: 35 }}>
                  Devenez Alfred
                </Typography>
              ...
              <div className="steps">
                <div className="step1">
                  ...
                  <Typography>Sélectionnez le premier service que vous souhaitez offrir. Vous pourrez ajouter autant de services que vous le souhaitez dans votre boutique.</Typography>
                </div>
                <div className="step2">
          ...
          <Wizard.Page>
            <Grid container className={classes.cardContainer} style={{ display: 'flex', justifyContent: 'start' }}>
              <div style={{ padding: '0rem 2rem 1rem 2rem' }}>
                <Typography variant="h6" style={{ marginBottom: '.5rem', marginTop: '1rem' }}>
                  Votre catégorie de service
                </Typography>
                <Typography>Commencez par sélectionner votre catégorie de service. Par exemple, si vous souhaitez réaliser un service de coiffure, sélectionnez la catégorie «Beauté et bien-être».</Typography>
              </div>
              <FieldArray
                name="categories"
                render={arrayHelpers =>
                  this.state.categories && this.state.categories.length > 0 ? (
                    <div style={{ padding: '.5rem 2rem' }}>
                      ...
                      <div>
                        ...
                        <Typography>Sélectionnez maintenant le service que vous souhaitez proposer dans la catégorie sélectionnée. Vous pourrez choisir les prestations que vous souhaitez proposer dès la prochaine étape !</Typography>
                        <Typography>
                      ...
                      <div style={{ marginTop: '1rem' }}>
                        {arrayHelpers.form.values.categories && arrayHelpers.form.values.categories.length > 0 && this.state.loading === false && this.state.checked_presta === true ? (
                          ...
                          <Loader type="TailSpin" color="#2FBCD3" height={100} width={100} style={{ textAlign: 'center' }} />
                        ) : (
                          <Typography align="center" style={{ fontSize: 15, marginTop: '2rem', color: '#F8727F' }}>
                            Sélectionnez votre catégorie pour afficher les services disponibles
                          </Typography>
                        )}
                      </div>
                    </div>
              ...
              <Field>
                {({ form }) => {
                  return form.values.services && form.values.services.length > 0 ? (
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          services.map((service, index) => {
                            ...
                            axios.get(`${url}myAlfred/api/service/${service.value}`).then(res => {
                              let servCompObj = {
                              ...
                              axios.get(`${url}myAlfred/api/prestation/${service.value}`).then(res => {
                                res.data.map(filters => {
                                  ...
                                  uniqueIdFilters = Array.from(new Set(servCompObj.filters.map(a => a.id))).map(id => {
                                    return servCompObj.filters.find(a => a.id === id);
                                  });
                                  servCompObj.filters = uniqueIdFilters;

                                  axios.get(`${url}myAlfred/api/prestation/${service.value}/${filterObj.id}`).then(res => {
                                    res.data.map(prestation => {
                                      ...
                                      servCompObj.filters.map(p => {
                                        if (p.id === prestationObj.filterId) {
                                          ...
                                          uniqueIdPrestations = Array.from(new Set(p.prestations.map(a => a.id))).map(id => {
                    ...
                    <div style={{ padding: '0 2rem 1rem 2rem' }}>
                      <Button color="primary" style={{ marginTop: '3rem', color: 'white', borderRadius: 8 }} variant="contained" type="button" disabled={true}>
                        Je valide ce service
                      </Button>
                    </div>
                  );
                  ...
                  return this.state.allInOneServ && this.state.allInOneServ.length > 0 ? (
                    <React.Fragment>
                      <div style={{ textAlign: 'justify' }}>
                        <Typography variant="h6" style={{ marginBottom: '.5rem' }}>
                          Paramétrez vos prestations<span style={{ color: '#F8727F' }}>*</span>
                        </Typography>
                        <Typography>Indiquez les prestations que vous souhaitez réaliser. Pour chacune, indiquez votre tarif et le mode de facturation que vous souhaitez appliquer.</Typography>
                      </div>
                            ...
                            <div>
                              <Grid>
                                {s.filters.map((f, indexf) => {
                                  return (
                                    <Grid item xs={12} key={f.id} className={classes.prestationsPres}>
                                  ...
                                  <Field
                                    render={({ form }) => {
                                      if (s.location.client === false) {
                                        if (form.values.createShop.option_presta_user !== false) {
                                          form.setFieldValue('createShop.option_presta_user', false);
                                        }
                                        return '';
                                      } else
                                        return (
                                  ...
                                  <Field
                                    render={({ form }) => {
                                      if (s.location.alfred === false) {
                                        if (form.values.createShop.option_presta_client != false) {
                                          form.setFieldValue('createShop.option_presta_client', false);
                                        }
                                        return '';
                                      } else
                                        return (
                                  ...
                                  <Field
                                    render={({ form }) => {
                                      if (s.location.visio === false) {
                                        if (form.values.createShop.option_presta_visio !== false) {
                                          form.setFieldValue('createShop.option_presta_visio', false);
                                        }
                                        return '';
                                      } else
                                        return (
                              ...
                              <div>
                                ...
                                <Typography style={{ marginBottom: '1rem' }}>Les frais de déplacement s'appliquent pour toutes les prestations réalisées à l'adresse de préstation indiquée par votre client. Si vous choisissez d'appliquer des frais de déplacements, ils seront automatiquement appliqués lors de la réservation.</Typography>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', width: '35%', justifyContent: 'center', marginTop: '-2%' }}>
                                      ...
                                      >
                                        <CssTextFieldOptions
                                          ...
                                          inputProps={{
                                            endAdornment: <InputAdornment position="start">€</InputAdornment>,
                                            className: classes.inputTextFielpickd
                                          }}
                                           onChange={e => {
                              ...
                              <div>
                                ...
                                <Typography style={{ marginBottom: '1rem' }}>Les options vous permettent de proposer des prestations complémentaires à vos clients. Dans le cadre de prestation de repassage par exemple, vous pouvez proposer à votre client de procéder au retrait et à la livraison du linge.</Typography>
                                <form noValidate autoComplete="off">
                                  <div
                                    style={{
                                      display: 'flex',
                                  ...
                                  >
                                    ...
                                    <div className={classes.contentCheckBox} style={{ width: '60%' }}>
                                      <label style={{ padding: '1%' }}>Retrait & livraison</label>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', width: '35%', justifyContent: 'center', marginTop: '-2%' }}>
                                      <div
                                        style={{
                              ...
                              {s.equipments.length === 0 ? null : (
                                <React.Fragment>
                                  <div>
                                    ...
                                    <Typography style={{ marginBottom: '1rem' }}>Sélectionnez les produits et le matériel que vous fournissez dans le cadre de vos prestations de service.</Typography>
                                  </div>
                                  <div>
                                    <Grid container>
                              ...
                              <div>
                                <Typography variant="h6" style={{ marginBottom: '.5rem' }}>
                                  Définissez votre montant minimum de réservation <span style={{ color: '#F8727F' }}>*</span>
                                </Typography>
                                <Typography>Le montant minimum de réservation correspond au panier minimum requis pour réserver ce service. Si vous indiquez un montant de 10€, les clients ne pourront pas réserver vos services si la somme des prestations n’atteint pas ce montant.</Typography>
                              ...
                              <div>
                                <Typography variant="h6" style={{ marginBottom: '.5rem' }}>
                                  Renseignez votre périmètre d’intervention <span style={{ color: '#F8727F' }}>*</span>
                                </Typography>
                                <Typography>Votre périmètre d’intervention est la zone dans laquelle vous souhaitez réaliser vos services. Par défaut, nous utilisons la ville de votre profil comme référence. Cette adresse ne vous convient pas ? Vous pouvez changer votre ville de référence à tout moment !</Typography>
                                <FormControlLabel
                                  control={
                                    <Checkbox
                                ...
                                />
                                {this.state[`userCityClicked${index}`] === true ? <AddressFinder formikCtx={arrayHelpers} index={index} /> : null}
                                <div style={{ padding: '1rem 0' }}>
                                  <Typography style={{ marginBottom: '1.5rem', fontSize: 17 }}>Définissez le périmètre que vous souhaitez couvrir :</Typography>
                                  <InputRange formatLabel={value => `${value}km`} step={1} maxValue={200} minValue={1} value={arrayHelpers.form.values.submission[index].perimeter} onChange={inputRangeValue => arrayHelpers.form.setFieldValue(`submission[${index}].perimeter`, inputRangeValue)} />
                                </div>
                              </div>
                              <hr style={{ margin: '1rem 0' }} />
                              <div>
                                <Typography variant="h6" style={{ marginBottom: '.5rem' }}>
                                  Indiquez votre délai de prévenance <span style={{ color: '#F8727F' }}>*</span>
                                </Typography>
                                <Typography>Le délai de prévenance correspond au délai nécessaire entre la réservation et la réalisation du service. Par exemple, si vous indiquez un délai de 24 heures, un client pourra réserver votre service 24 heures avant votre intervention.</Typography>
                                <Grid item xs={12} className={classes.delayDivResponsive}>
                                  <Field
                                    ...
                                    render={() => {
                                      return (
                                        <React.Fragment>
                                          <div
                                            style={{ width: 30, height: 30, borderRadius: '50%', border: '1px solid #2FBCD3', textAlign: 'center', lineHeight: 1.6, cursor: 'pointer', display: 'inline-block', marginRight: 25 }}
                                            onClick={() => {
                                          ...
                                          <div
                                            style={{ width: 30, height: 30, borderRadius: '50%', border: '1px solid #2FBCD3', textAlign: 'center', lineHeight: 1.6, cursor: 'pointer', display: 'inline-block', marginLeft: 25, marginRight: '5%' }}
                                            onClick={() => {
                                  ...
                                  <Field
                                    ...
                                    render={({ field, form }) => {
                                      return (
                                        <TextField {...field} value={field.value} style={{ width: '30%' }} className={classes.selectDelayInputRepsonsive} select margin="dense" variant="outlined" label="Heures / jours / semaines" InputLabelProps={{ shrink: form.values.submission[index].delayBeforeShopDWM !== null }}>
                              ...
                              <div>
                                ...
                                <Typography>
                                  Décrivez votre expertise et précisez votre service ! Mettez en évidence vos compétences et votre expertise dans ce service. Vous pouvez également préciser la façon dont les utilisateurs doivent indiquer les quantités pour réserver. Par exemple, si vous proposez un service de confection de tapis, vous pouvez indiquer les heures nécessaires pour différentes dimension de tapis. Précisez tout ce qui peut aider votre client à réserver correctement votre service !
                                </Typography>
                                <Field
                                  name={`submission[${index}].descService`}
                                  render={({ field }) => {
                                    return <TextField {...field} value={field.value} id="outlined-multiline-static" label="Description du service" multiline rows="6" margin="normal" variant="outlined" style={{ width: '100%' }} />;
                                  }}
                                />
                              ...
                              <div>
                                <Typography variant="h6" style={{ marginBottom: '.5rem' }}>
                                  Votre expérience, vos certifications & diplômes
                                </Typography>
                                <Typography>Si vous possédez des certifications et/ou diplômes pour ce service, mettez les en avant ! Après vérification par My-Alfred, vous aurez le statut d’Alfred certifié et/ou diplômé sur ce service.</Typography>
                                    ...
                                    {arrayHelpers.form.values.submission[index].diploma.label !== null && arrayHelpers.form.values.submission[index].diploma.year !== null && arrayHelpers.form.values.submission[index].diploma.diploma !== null ? (
                                      <React.Fragment>
                                        <div style={{ border: '1px solid lightgrey', width: '50%', textAlign: 'center', marginBottom: '1.5rem', position: 'relative' }}>
                                          ...
                                          <p>
                                            {arrayHelpers.form.values.submission[index].diploma.label} | {arrayHelpers.form.values.submission[index].diploma.year}
                                          </p>
                                        </div>
                                      </React.Fragment>
                                    ) : null}
                                        ...
                                        <Grid container>
                                          ...
                                          <Grid item xs={12}>
                                            <Field
                                              ...
                                              render={({ field }) => {
                                                return (
                                                  <TextField {...field} value={field.value || ''} style={{ width: '50%', marginRight: '5%' }} className={classes.inputDiplomaCertifResp} label="Année d'obtention" margin="dense" variant="outlined" select InputLabelProps={{ shrink: arrayHelpers.form.values.submission[index].diploma.year !== null }}>
                                                    {dates.map(date => {
                                                      return (
                                                        <MenuItem key={date} style={{ zIndex: 9999 }} value={date}>
                                                          {date}
                                                        </MenuItem>
                                                      );
                                                    })}
                                    ...
                                    {arrayHelpers.form.values.submission[index].certification.label !== null && arrayHelpers.form.values.submission[index].certification.year !== null && arrayHelpers.form.values.submission[index].certification.certification !== null ? (
                                      <React.Fragment>
                                        <div style={{ border: '1px solid lightgrey', width: '50%', textAlign: 'center', marginBottom: '1.5rem', position: 'relative' }}>
                                          ...
                                          <p>
                                            {arrayHelpers.form.values.submission[index].certification.label} | {arrayHelpers.form.values.submission[index].certification.year}
                                          </p>
                                        </div>
                                      </React.Fragment>
                                    ) : null}
                                        ...
                                        <Grid container>
                                          ...
                                          <Grid item xs={12}>
                                            <Field
                                              ...
                                              render={({ field }) => {
                                                return (
                                                  <TextField {...field} value={field.value || ''} style={{ width: '50%', marginRight: '5%' }} className={classes.inputDiplomaCertifResp} label="Année d'obtention" margin="dense" variant="outlined" select InputLabelProps={{ shrink: arrayHelpers.form.values.submission[index].certification.year !== null }}>
                                                    {dates.map(date => {
                                                      return (
            ...
            <Grid container className={classes.cardContainer} style={{ overflow: 'hidden' }}>
              <div className={classes.newContainer}>
                ...
                <Grid container>
                  ...
                  <Typography style={{ marginBottom: 20, fontFamily: 'helvetica', fontSize: '1rem', fontWeight: 400 }}>Il se peut que vous ayez moins de réservations si vous ajoutez des conditions. Les personnes qui ne répondent pas à vos critères peuvent quand même envoyer une demande.</Typography>
                          ...
                          <FormControlLabel
                            ...
                            label={
                              <React.Fragment>
                                ...
                                <p style={{ marginTop: 0, fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Adresse email et numéro de téléphone confirmés</p>
                              </React.Fragment>
                            }
                          />
                        );
                          ...
                          <FormControlLabel
                            ...
                            label={
                              <React.Fragment>
                                ...
                                <p style={{ marginTop: 0, fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Ces utilisateurs ont fourni une photo de profil.</p>
                              </React.Fragment>
                            }
                          />
                        );
                          ...
                          <FormControlLabel
                            ...
                            label={
                              <React.Fragment>
                                ...
                                <p style={{ marginTop: 0, fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Ces utilisateurs ont vérifié leur pièce d'identité.</p>
                              </React.Fragment>
                            }
                          />
                        );
                          ...
                          <FormControlLabel
                            ...
                            label={
                              <React.Fragment>
                                ...
                                <Typography style={{ marginTop: 0, fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Ces utilisateurs ont déjà utilisé des services avec My-Alfred, sont recommandés par d'autres Alfred et non pas reçu de commentaires négatifs.</Typography>
                              </React.Fragment>
                            }
                          />
                        );
                ...
                <Grid container>
                  <Typography variant="h6" style={{ marginBottom: '10' }}>
                    Votre message de bienvenue validant votre réservation <span style={{ color: '#F8727F' }}>*</span>
                  </Typography>
                  ...
                  <Grid item style={{ marginTop: 15, width: '100%' }}>
                    <Field
                      ...
                      render={({ field, form }) => {
                        return <TextField {...field} id="outlined-multiline-static" label={'Votre message'} multiline rows="6" margin="normal" variant="outlined" style={{ width: '100%' }} value={form.values.createShop.welcome_message} />;
                      }}
                    />
                ...
                <Grid container>
                  ...
                  <Typography style={{ fontFamily: 'helvetica', width: '100%' }}>Choisissez vos conditions en cas d'annulation de la part des utilisateurs.</Typography>
                  <Grid item style={{ width: '100%', marginTop: 10, marginBottom: 10, marginLeft: 20 }}>
                    <Field
                      render={({ form }) => {
                        return (
                          <FormControlLabel
                            ...
                            label={
                              <React.Fragment>
                                ...
                                <Typography style={{ marginTop: 0, fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Remboursement intégral jusqu’à un jour avant la prestation</Typography>
                              </React.Fragment>
                            }
                          />
                        );
                          ...
                          <FormControlLabel
                            ...
                            label={
                              <React.Fragment>
                                ...
                                <Typography style={{ marginTop: 0, fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Remboursement intégral jusqu’à 5 jours avant la prestation</Typography>
                              </React.Fragment>
                            }
                          />
                        );
                          ...
                          <FormControlLabel
                            ...
                            label={
                              <React.Fragment>
                                ...
                                <p style={{ marginTop: 0, fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Remboursement intégral jusqu’à 10 jours avant la prestation</p>
                              </React.Fragment>
                            }
                          />
                        );
                  ...
                  <Grid container className={classes.cardContainer} style={{ overflow: 'hidden' }}>
                    <div className={classes.newContainer}>
                      <Grid container>
                        ...
                        <Typography style={{ fontFamily: 'helvetica', fontSize: '1rem', fontWeight: 400 }}>Ces informations ne seront pas visibles par les utilisateurs. Un profil vérifié est plus engageant pour les utilisateurs !</Typography>
                        <Grid item xs={12}>
                          <Typography style={{ fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Vos informations ne seront pas visibles par les autres utilisateurs</Typography>
                        </Grid>
                          ...
                          <Grid container>
                            ...
                            >
                              <FormControl component="fieldset">
                                ...
                                >
                                  <FormControlLabel value="passport" control={<Radio color="primary" />} label="Passeport" labelPlacement="start" />
                                  <FormControlLabel value="id" control={<Radio color="primary" />} label="Carte d'identité" labelPlacement="start" />
                                </RadioGroup>
                              </FormControl>
                                        ...
                                        return (
                                          <React.Fragment>
                                            ...
                                            {form.values.createShop.id_recto === null && typeof this.state.userIdCardRecto !== 'undefined' && this.state.userIdCardRecto !== null && typeof this.state.userIdCardVerso !== 'undefined' ? (
                                              <React.Fragment>
                                                <span>{this.state.userIdCardRecto.slice(0, 10) + '...'}</span>
                                                <Clear
                                                  color="secondary"
                                                  style={{ cursor: 'pointer' }}
                                                  onClick={() => {
                                            ...
                                            {form.values.createShop.id_recto !== null ? (
                                              <React.Fragment>
                                                <span>{form.values.createShop.id_recto.name.substr(0, 10) + '...'}</span>
                                                <Clear color="secondary" style={{ cursor: 'pointer' }} onClick={() => form.setFieldValue('createShop.id_recto', null)} />
                                              </React.Fragment>
                                            ) : null}
                                        ...
                                        return (
                                          <React.Fragment>
                                            ...
                                            {form.values.createShop.id_verso === null && typeof this.state.userIdCardVerso !== 'undefined' && this.state.userIdCardVerso !== null ? (
                                              <React.Fragment>
                                                <span>{this.state.userIdCardVerso.slice(0, 10) + '...'}</span>
                                                <Clear
                                                  color="secondary"
                                                  style={{ cursor: 'pointer' }}
                                                  onClick={() => {
                                            ...
                                            {form.values.createShop.id_verso !== null ? (
                                              <React.Fragment>
                                                <span>{form.values.createShop.id_verso.name.substr(0, 10) + '...'}</span>
                                                <Clear color="secondary" style={{ cursor: 'pointer' }} onClick={() => form.setFieldValue('createShop.id_verso', null)} />
                                              </React.Fragment>
                                            ) : null}
                                    ...
                                    render={({ form }) => {
                                      return (
                                        <React.Fragment>
                                          ...
                                          {form.values.createShop.id_recto !== null ? (
                                            <React.Fragment>
                                              <span>{form.values.createShop.id_recto.name.substr(0, 10) + '...'}</span> <Clear color="secondary" style={{ cursor: 'pointer' }} onClick={() => form.setFieldValue('createShop.id_recto', null)} />
                                            </React.Fragment>
                                          ) : null}
                                          <ErrorMessage name="createShop.id_recto" render={msg => <div style={{ color: 'red' }}>{msg}</div>} />
                                        </React.Fragment>
                      ...
                      <Grid container className={classes.checkboxespart}>
                        <Grid container>
                              ...
                              render={({ form }) => {
                                return (
                                  <FormControlLabel
                                    ...
                                    label={
                                      <Typography variant="h6" style={{ fontSize: '1rem', fontWeight: 400, marginTop: 15, marginBottom: 10 }}>
                          ...
                          <Grid item xs={11}>
                            <Typography style={{ marginLeft: '2.5rem', fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>En tant que particulier, vous pouvez rendre des services occasionnels sur My-Alfred. Si votre activité devient régulière, un statut professionnel (micro-entrepreneur,...) s’impose. Il est également requis pour certains secteurs d’activité réglementés.</Typography>
                          </Grid>
                        </Grid>
                        ...
                        <Grid container style={{ marginTop: 10 }}>
                              ...
                              render={({ form }) => {
                                return (
                                  <FormControlLabel
                                    ...
                                    label={
                                      <Typography variant="h6" style={{ marginBottom: '.5rem', fontSize: '1rem', fontWeight: 400 }}>
                          ...
                          <Grid item xs={11}>
                            <Typography style={{ marginLeft: '2.5rem', fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Un statut professionnel est nécessaire pour les métiers réglementés et permet une activité régulière sur My-Alfred. Seuls les professionnels peuvent proposer leurs services aux entreprises qui ont besoin d’une facture. Un statut professionnel est requis dès lors que votre activité devient régulière.</Typography>
                            {isProfessional ? (
                              <React.Fragment>
                                <Field
                                  render={({ form }) => {
                                  ...
                                  render={({ form }) => {
                                    return (
                                      <FormControlLabel
                                        ...
                                        label={'Je certifie sur l’honneur qu’il s’agit bien de mon entreprise.'}
                                      />
                                    );
                                  }}
                                />
                      ...
                      <Grid container>
                        ...
                        <Typography style={{ fontSize: '1rem', fontWeight: 400, fontFamily: 'helvetica' }}>Dans le cadre des prestations effectuées via My-Alfred, vous devez respecter toutes les obligations légales et réglementaires (fiscales, sociales, comptables, administratives etc...) liées à votre statut.</Typography>
                        <Grid container>
                          <Grid item>
                            <Field
                              render={({ form }) => {
                                return (
                                  <FormControlLabel
                                    ...
                                    label={'Je m’engage à respecter toutes les obligations légales correspondant à mon statut.'}
                                  />
                                );
                              }}
                            />
            ...
            return props.option.map((select, index) => {
              return (
                <React.Fragment key={index}>
                  <p>
                    Prestation(s) pour : {select.serviceLabel} ({select.filterLabel})
                  </p>
                  <label>
                    {select.label}
                    <input
                      ...
                      onChange={() => {
                        ...
                        if (form.values.prestations.includes(select.label)) {
                          const nextValue = form.values.prestations.filter(value => value !== select.label);
