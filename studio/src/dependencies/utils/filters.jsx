...
export const buildFilter = (dataSourceId, filterAttributes, componentsValues) => {
  // componentsValues store comp-XXX_0_1_2 while componentName is the studio's one (i.e. comp-XXX)
  const getComponentValue= compId => {
    const val=Object.entries(componentsValues).find(([comp, value]) => comp.startsWith(compId))?.[1]
    return val
  }
  const log=/comp-/.test(dataSourceId) ? console.log : () => {}
  log('Building filter for', dataSourceId)
  log('components values', componentsValues)
