...
class ConfirmPayement extends React.Component {
    ...
    componentDidMount() {
        ...
        this.setState({
          ...
          date: moment(localStorage.getItem('date')).format('DD/MM/YYYY'),
          hour: moment(localStorage.getItem('hour')).format('HH:mm'),
                        ...
                        {/*Contenu à Gauche*/}
                            ...
                            <Grid item md={5} xs={12} style={{textAlign: 'left',margin: '0 auto', float:'right', paddingLeft:'3%'}}>
                                <div style={{margin: '20px 11%', marginTop: '5%',width: '90%'}}></div>
                                    ...
                                    <h2 style={{fontSize: '2rem',color: 'rgba(84,89,95,0.95)',letterSpacing: -1, fontWeight: '100'}}>Détail de votre réservation</h2>
                                        </Grid>
                                    </Grid>
                                <br></br>
                                    <Grid container> 
                                    <Grid item xs={5} style={{}}>
                                    <img src="../../static/basicavatar.png" style={{borderRadius: '50%',marginLeft:'auto',marginRight:'auto',zIndex:501, minWidth: '137px', maxWidth: '137px', maxHeight: '137px', minHeight: '137px',}} alt={'picture'}/>

                                    </Grid>
                                    
                                    <Grid item xs={5} style={{}}>
                                    
                                            <h3 style={{fontSize: '1.6rem',color: 'rgba(84,89,95,0.95)',letterSpacing: -1, fontWeight: 'bold'}}>A propos de {user.firstname}</h3>
                                ...
                                <hr></hr>
                                    <Grid container>
                                    <h3 style={{fontSize: '1.6rem',color: 'rgba(84,89,95,0.95)',letterSpacing: -1, fontWeight: 'bold'}}>A propos de votre réservation</h3>
                                    <Grid item xs={12} style={{}}>
                                    
                                   
                                    <Grid item xs={3} style={{width:'30%', float:'left', paddingTop:15}}><img src="../../static/calendarreservation.svg" width={'35%'}/></Grid>
                                    <Grid item xs={9} style={{width:'70%',}}><p>Date et heure de la prestation:</p> <p>{this.state.date} - {this.state.hour}</p></Grid>
                                    
                            </Grid>
                            <Grid item xs={12} style={{}}>

                            <Grid item xs={3} style={{width:'30%', float:'left', paddingTop:15}}><img src="../../static/mapmarker.svg" width={'35%'}/></Grid>
                                    <Grid item xs={9} style={{width:'70%',}}><p>Adresse de la prestation:</p> <p>12 place cauchoise, Rouen 76000.</p></Grid>
                                    ...
                                    <h3 style={{fontSize: '1.6rem',color: 'rgba(84,89,95,0.95)',letterSpacing: -1, fontWeight: 'bold'}}>Paiement</h3>
                                    <Grid xs={12}>
                                      {this.state.prestations.length ?
                                        this.state.prestations.map(prestation => {
                                          return (
                                            <>
                                              <Grid item xs={9} style={{width:'90%', float:'left'}}><p>{prestation.value}X {prestation.name}</p></Grid>
                                              <Grid item xs={3} style={{width:'10%', float:'right'}}><p>{prestation.price * prestation.value}€</p></Grid>
                                            </>
                                          )
                                      }): null}
                                    ...
                                    <Grid item xs={9} style={{width:'90%', float:'left'}}><p>Frais de service</p></Grid>
                                    <Grid item xs={3} style={{width:'10%', float:'right'}}> <p>{this.state.fees}€</p></Grid>
                                    <Grid item xs={9} style={{width:'90%', float:'left', fontSize:25, fontWeight:600, color:'#2FBCD3'}}><p>Total</p></Grid>
                                    <Grid item xs={3} style={{width:'10%', float:'right', fontWeight:600, fontSize:25,color:'#2FBCD3'}}> <p>{this.state.grandTotal}€</p>

                                    <Grid style={{float:'right'}} item xs={12}> <Button  color={"secondary"} variant={"contained"} style={{color:"white", fontSize:'16px', width:"100%", paddingLeft:'20px', paddingRight:'20px', marginBottom:50, marginRight:20}}>Payer</Button>
                                    </Grid>
                                     </Grid>
