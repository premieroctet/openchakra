...
describe('Test quotations', () => {
  ...
  it('must return status', async() => {
    ...
    await Mission.findOneAndUpdate({}, {$set: {billing_sent_date: moment()}})
    expect((await Mission.findOne()).status).toEqual(MISSION_STATUS_BILL_SENT)
    await Mission.findOneAndUpdate({}, {$set: {customer_accept_billing_date: moment()}})
    expect((await Mission.findOne()).status).toEqual(MISSION_STATUS_FINISHED)
    await Mission.findOneAndUpdate({}, {$set: {customer_accept_billing_date: null, customer_refuse_billing_date: moment()}})
