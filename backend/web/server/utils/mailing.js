...
const SMS_CONTENTS = {
  [SIB_IDS.NEW_BOOKING_MANUAL]:
    "{{ params.client_firstname }} a effectué une demande de réservation de votre service {{ params.service_label }}",
  [SIB_IDS.CONFIRM_PHONE]:
    "Pour valider votre numéro de téléphone portable, merci de saisir le code d'activation suivant : {{ params.sms_code }}",
  [SIB_IDS.ASKING_INFO]:
    "{{ params.client_firstname }} a effectué une demande d'information pour votre service {{ params.service_label }}",
  [SIB_IDS.BOOKING_CANCELLED_BY_CLIENT]:
    "Malheureusement, {{ params.client_firstname }} a annulé la réservation de votre service {{ params.service_label }}",
  [SIB_IDS.TRANSFER_TO_ALFRED]:
    "Un versement de {{ params.total_revenue }} a été effectué pour votre service {{ params.service_label }}",
  [SIB_IDS.BOOKING_CANCELLED_BY_ALFRED]:
    "Malheureusement, {{ params.alfred_firstname }} a annulé votre réservation du service {{ params.service_label }}",
  [SIB_IDS.ASKINFO_PREAPPROVED]:
    "{{ params.alfred_firstname }} a pré approuvé la réservation de votre service {{ params.service_label }}",
  [SIB_IDS.BOOKING_REFUSED_2_CLIENT]:
    "Malheureusement, {{ params.alfred_firstname }} a refusé votre réservation du service {{ params.service_label }}",
  [SIB_IDS.BOOKING_CONFIRMED]:
    "{{ params.alfred_firstname }} a confirmé votre réservation de son service {{ params.service_label }}",
  [SIB_IDS.NEW_BOOKING]:
    "{{ params.client_firstname }} a réservé votre service {{ params.service_label }}",
  [SIB_IDS.BOOKING_EXPIRED_2_CLIENT]:
    "Votre réservation du service {{ params.service_label }} par {{ params.alfred_firstname }} est expirée",
  [SIB_IDS.BOOKING_EXPIRED_2_ALFRED]:
    "La réservation de votre service {{ params.service_label }} par {{ params.client_firstname }} est expirée"
};
...
const sendNotification = (
  ...
  const msg = `Sending notif ${notif_index} to ${destinee.email}(${
    destinee._id
  }) using ${JSON.stringify(params)}`;
  ...
  if (
    !enable_mails &&
    !is_production() &&
    ALLOW_EMAILS.test(destinee.email || "")
  ) {
    console.log(
      "Mailing disabled except for my-alfred.io mails on validation platform"
    );
    enable_mails = true;
  }
...
const sendVerificationMail = (user, req) => {
  sendNotification(SIB_IDS.CONFIRM_EMAIL, user, {
    link_confirmemail: new URL(
      `/validateAccount?user=${user._id}`,
      getHostUrl()
    ),
...
const sendVerificationSMS = user => {
  sendNotification(SIB_IDS.CONFIRM_PHONE, user, {
...
const sendShopDeleted = user => {
  sendNotification(SIB_IDS.SHOP_DELETED, user, {
    user_firstname: user.firstname
  });
};

const sendBookingConfirmed = booking => {
  sendNotification(SIB_IDS.BOOKING_CONFIRMED, booking.user, {
...
const sendBookingCancelledByAlfred = (booking, req) => {
  sendNotification(SIB_IDS.BOOKING_CANCELLED_BY_ALFRED, booking.user, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking),
...
const sendBookingCancelledByClient = booking => {
  sendNotification(SIB_IDS.BOOKING_CANCELLED_BY_CLIENT, booking.alfred, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking)
...
const sendLeaveCommentForClient = booking => {
  sendNotification(
    ...
    booking.alfred,
    {
      ...
      alfred_firstname: booking.alfred.firstname,
      service_label: booking.service,
      link_reviewsclient: new URL(
        `/evaluateClient?booking=${booking._id}&id=${booking.serviceUserId}&client=${booking.user._id}`,
        getHostUrl()
      )
    }
  );
...
const sendLeaveCommentForAlfred = booking => {
  sendNotification(
    ...
    booking.user,
    {
      ...
      alfred_firstname: booking.alfred.firstname,
      service_label: booking.service,
      link_reviewsalfred: new URL(
        `/evaluate?booking=${booking._id}&id=${booking.serviceUserId}`,
        getHostUrl()
      )
    }
  );
...
const sendResetPassword = (user, token) => {
  const url = new URL(`/resetPassword?token=${token}`, getHostUrl());
  sendNotification(SIB_IDS.RESET_PASSWORD, user, {
    user_firstname: user.firstname,
...
const sendBookingExpiredToAlfred = booking => {
  sendNotification(SIB_IDS.BOOKING_EXPIRED_2_ALFRED, booking.alfred, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking)
...
const sendBookingExpiredToClient = booking => {
  sendNotification(SIB_IDS.BOOKING_EXPIRED_2_CLIENT, booking.user, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking),
...
const sendBookingDetails = booking => {
  sendNotification(SIB_IDS.BOOKING_DETAILS, booking.user, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking),
...
const sendBookingInfosRecap = (booking, req) => {
  sendNotification(SIB_IDS.BOOKING_INFOS_RECAP, booking.user, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking),
    total_cost: parseFloat(booking.amount).toFixed(2),
    link_requestinformation: new URL(
      `/reservations/reservations?id=${booking._id}`,
      getHostUrl()
    )
  });
};
...
const sendNewBooking = (booking, req) => {
  sendNotification(SIB_IDS.NEW_BOOKING, booking.alfred, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking),
    total_revenue: parseFloat(booking.alfred_amount).toFixed(2),
    link_showreservation: new URL(
      `/reservations/reservations?id=${booking._id}`,
      getHostUrl()
    )
  });
...
const sendShopOnline = (alfred, req) => {
  sendNotification(SIB_IDS.SHOP_ONLINE, alfred, {
    alfred_firstname: alfred.firstname,
    link_manageshop: new URL(
      `/profile/services?user=${alfred._id}`,
      getHostUrl()
    )
  });
};
...
const sendBookingRefusedToClient = (booking, req) => {
  sendNotification(SIB_IDS.BOOKING_REFUSED_2_CLIENT, booking.user, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking),
...
const sendBookingRefusedToAlfred = booking => {
  sendNotification(SIB_IDS.BOOKING_REFUSED_2_ALFRED, booking.alfred, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking)
...
const sendAskingInfo = (booking, req) => {
  sendNotification(SIB_IDS.ASKING_INFO, booking.alfred, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking),
...
const sendNewMessageToAlfred = (booking, chatroom_id, req) => {
  sendNotification(SIB_IDS.NEW_MESSAGE_ALFRED, booking.alfred, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    link_showclientmessage: new URL(
      `/profile/messages?user=${booking.alfred._id}&relative=${booking.user._id}`,
      getHostUrl()
    )
  });
};
...
const sendNewMessageToClient = (booking, chatroom_id, req) => {
  sendNotification(SIB_IDS.NEW_MESSAGE_CLIENT, booking.user, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    link_showalfredmessage: new URL(
      `/profile/messages?user=${booking.user._id}&relative=${booking.alfred._id}`,
      getHostUrl()
    )
  });
};
...
const sendAskInfoPreapproved = (booking, req) => {
  sendNotification(SIB_IDS.ASKINFO_PREAPPROVED, booking.user, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    link_confirmbooking: new URL(
      `/reservations/reservations?id=${booking._id}`,
      getHostUrl()
    )
  });
...
const sendNewBookingManual = (booking, req) => {
  sendNotification(SIB_IDS.NEW_BOOKING_MANUAL, booking.alfred, {
    client_firstname: booking.user.firstname,
    alfred_firstname: booking.alfred.firstname,
    service_label: booking.service,
    service_datetime: booking_datetime_str(booking),
...
const sendAlert = (user, subject, message) => {
  sendNotification(SIB_IDS.ALERT, user, {
...
const sendB2BAccount = (user, email, role, company, token, req) => {
  sendNotification(SIB_IDS.B2B_ACCOUNT_CREATED, user, {
    user_firstname: user.firstname,
    role: role,
    company: company,
    user_email: email,
    link_initiatenewpassword: new URL(
...
const sendBillingToAlfred = booking => {
  sendNotification(BILLING_2_ALFRED, [booking.alfred, booking.user.email], {
