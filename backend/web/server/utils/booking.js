...
const computeServiceDistance = ({ location, serviceUser, customer }) => {
  ...
  // Check distance
  const addr =
    serviceUser.customer_booking?.address || customer.billing_address;
  const distance = computeDistanceKm(
    addr.gps,
    serviceUser.user.billing_address.gps
  );
  return distance;
};
...
const createBooking = ({
  ...
  let bookData = {
    ...
    prestation_date: modifiedDate,
    cpf_booked: cpf,
    customer_booking: customerBookingId
  };
  ...
  return ServiceUser.findById(serviceUserId)
    ...
    .then(result => {
      ...
      const prestas = Object.entries(prestations).map(([key, count]) => {
        const prestation = serviceUser.prestations.find(
          p => p._id.toString() == key
        );
        return {
          name: prestation.prestation.label,
      ...
      const address =
        location == LOCATION_CLIENT
          ...
          : location == LOCATION_ALFRED
          ? serviceUser.user.billing_address
          : null;
      bookData = {
        ...bookData,
      ...
      const distance = computeServiceDistance({
        location,
        serviceUser,
        customer
      });
      const payment = serviceUser.customer_booking
        ? new PlatformPayment()
        : new MarketplacePayment();
      return payment.compute({
        serviceUser,
        prestations,
        location,
        cpf,
    ...
    .then(prices => {
      const status = informationRequest
        ? BOOK_STATUS.INFO
        : customerBookingId
    ...
    .then(book => {
      return upsertChatroom(customer._id, serviceUser.user._id, book).then(
        () => book
      );
    })
    .then(book => {
