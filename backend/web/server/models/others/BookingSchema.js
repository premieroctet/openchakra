...
const BookingSchema = new Schema(
  {
    ...
    address: {
      type: AddressSchema,
      required: function() {
        return [LOCATION_CLIENT, LOCATION_ALFRED].includes(this.location);
      }
    },
    location: {
      type: String,
    ...
    customer_fees: [BookingFee],
    provider_fees: [BookingFee],
    status: {
      type: String,
    ...
    elearning_link: {
      ...
      required: function() {
        return (
          this.location == LOCATION_ELEARNING &&
          this.status == BOOK_STATUS.CONFIRMED
        );
      }
    },
...
BookingSchema.virtual("calendar_display").get(function() {
  ...
  if (
    [BOOK_STATUS.CANCELLED, BOOK_STATUS.EXPIRED, BOOK_STATUS.REFUSED].includes(
      this.status
    )
  ) {
    return false;
...
BookingSchema.virtual("customer_fee").get(function() {
  return roundCurrency(
    lodash.sum((this.customer_fees || []).map(c => c.amount))
  );
});

BookingSchema.virtual("provider_fee").get(function() {
  return roundCurrency(
    lodash.sum((this.provider_fees || []).map(c => c.amount))
  );
});
