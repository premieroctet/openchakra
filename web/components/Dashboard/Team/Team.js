...
class Team extends React.Component {
  ...
  dialogRemove = classes => {
    ...
    return(
      ...
      >
        <MuiDialogTitle id="alert-dialog-title">{ReactHtmlParser(this.props.t('TEAM.dialog_remove_title'))}</MuiDialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            {ReactHtmlParser(this.props.t('TEAM.dialog_remove_question')) + selected.email} ?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
  ...
  dialogGroupe = classes => {
    ...
    return(
      <Dialog open={dialogGroupe} onClose={() => this.setState({dialogGroupe: false})} aria-labelledby="form-dialog-title" classes={{paper: classes.dialogPaper}}>
        <DialogTitle id="customized-dialog-title" onClose={() => this.setState({dialogGroupe: false})} >{mode === MICROSERVICE_MODE ? ReactHtmlParser(this.props.t('TEAM.dialog_group_add')) : ReactHtmlParser(this.props.t('TEAM.dialog_group_add_b2b'))}</DialogTitle>
        <DialogContent dividers>
          <Grid container spacing={2} style={{width: '100%', margin: 0}}>
            <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
              <h3>{mode === MICROSERVICE_MODE ? ReactHtmlParser(this.props.t('TEAM.dialog_groupe_title')) : ReactHtmlParser(this.props.t('TEAM.dialog_groupe_title_b2b'))}</h3>
            </Grid>
            <Grid item xl={12} lg={12} md={12} sm={12} xs={12} container spacing={2} style={{width: '100%', margin: 0}}>
              <Grid item xl={12} lg={12} sm={12} md={12} xs={12}>
                <TextField
                  label={ReactHtmlParser(this.props.t('TEAM.dialog_groupe_name'))}
                  name={'nameGroupe'}
                  value={nameGroupe}
                  variant={'outlined'}
                  classes={{root: classes.textField}}
              ...
              <Grid item xl={6} lg={6} sm={6} md={6} xs={6}>
                <TextField
                  label={ReactHtmlParser(this.props.t('TEAM.dialog_groupe_plafond'))}
                  name={'plafondGroupe'}
                  value={plafondGroupe}
                  type={'number'}
                  variant={'outlined'}
    ...
    return(
      <Grid container spacing={3} style={{marginTop: '3vh', width: '100%', margin: 0}}>
        <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
          <Grid style={{display: 'flex', alignItems: 'center'}}>
            <Grid>
              <h3>{ReactHtmlParser(this.props.t('TEAM.title'))}</h3>
            </Grid>
            <Grid>
          ...
          <Box>
            {
              admins.length > 0 ?
                ...
                <Grid>
                  <Typography>{ReactHtmlParser(this.props.t('TEAM.no_admin'))}</Typography>
                </Grid>
            }
          </Box>
        </Grid>
        <Grid item xl={12} lg={12} md={12} sm={12} xs={12}>
          <Grid style={{display: 'flex', alignItems: 'center'}}>
            <Grid>
              <h3>{mode === MICROSERVICE_MODE ? ReactHtmlParser(this.props.t('TEAM.micro_mode')) : ReactHtmlParser(this.props.t('TEAM.no_micro_mode'))}</h3>
            </Grid>
            <Grid>
          ...
          <Box>
            <Grid>
              {groups.length > 0 ?
                    ...
                    groups.map((res, index) => (
                      <>
                        <ListItem key={index}>
                          <ListItemText
                            primary={res.name}
                            secondary={res.budget ? `${res.budget}€ / ${BUDGET_PERIOD[res.budget_period]}` : ReactHtmlParser(this.props.t('TEAM.no_budget'))}
                          />
                          { consumed_budgets[res._id] ?
                            <ListItemText secondary={`${consumed_budgets[res._id]}€${ReactHtmlParser(this.props.t('TEAM.available'))}`} />
                            :
                            null
                          }
                          <ListItemSecondaryAction>
                ...
                :
                <Grid>
                  <Typography>{ReactHtmlParser(this.props.t('TEAM.no_department'))}</Typography>
                </Grid>
              }
            </Grid>
          </Box>
          ...
          groups.length > 0 ?
            <>
              <Grid item xl={12} lg={12} md={12} sm={12} xs={12} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                <Grid style={{display: 'flex', alignItems: 'center'}}>
                  <Grid>
                    <h3>{mode === MICROSERVICE_MODE ? ReactHtmlParser(this.props.t('TEAM.manager')) : ReactHtmlParser(this.props.t('TEAM.collaborateur'))}</h3>
                  </Grid>
                ...
                <Grid className={classes.searchFilterRightContainer}>
                  <Grid className={classes.searchFilterRightLabel}>
                    <Typography>{ReactHtmlParser(this.props.t('TEAM.filter'))}</Typography>
                  </Grid>
                  <Grid>
                    <FormControl>
                      <Select
                                    ...
                                    <ListItemSecondaryAction>
                                      {
                                        !groups.length > 0 ? null :
                                          <FormControl className={classes.formControl}>
                                            <InputLabel id="demo-simple-select-label">{ReactHtmlParser(this.props.t('TEAM.departement'))}</InputLabel>
                                            <Select
