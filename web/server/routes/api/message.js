...
router.get('/all', passport.authenticate('jwt', {session: false}), (req, res) => {
  ...
  Message.find({$or: [{sender: req.user.id}, {receiver: req.user.id}]})
    ...
    .catch(err => res.status(HTTP_CODES.NOT_FOUND).json({message: 'No message found'}))
})
...
router.delete('/:id', passport.authenticate('jwt', {session: false}), (req, res) => {
  Message.findById(req.params.id)
    ...
    .catch(err => res.status(HTTP_CODES.NOT_FOUND).json({messagenotfound: 'No message found'}))
})
